...
| CI analyze fails with `uri_does_not_exist` / `undefined_identifier` on `f_*` fixtures, local passes | Tests import GENERATED fixtures (`test/fixtures/generated/`, gitignored — absent on a fresh checkout). The analyze/test targets must depend on `fixtures` so it regenerates first. A dev machine has stale `generated/` lying around, hiding the gap. Same class as the format trap: CI's fresh checkout lacks generated prerequisites a dev machine accumulates. |
| CI format step fails but local `make analyze` passes | `dart format` output depends on each file's resolved **language version** (new arg-per-line style at SDK >=3.7, old wrap style below) — read from the package's pubspec SDK floor via `.dart_tool/package_config.json`. With no `package_config` (unresolved deps) the formatter falls back to the SDK's latest, a different style. `analyze.sh` resolves BOTH packages before formatting and formats each from its own root so the version is deterministic. Never format a package whose deps aren't resolved; never reorder format before pub get. |
