...
| Rust test speed | `make test-rust` runs the engines' suites through cargo-nextest (every test its own process across all cores; `cargo test` ran pdf_oxide's ~300 test binaries one after another) plus `cargo test --doc` for the doctests nextest skips, and links with mold on Linux. Same tests, same pass/fail; nextest is required, never a fallback, so "the suite passed" means one thing on every machine. nextest is a pinned, sha256-verified download (`NEXTEST_VERSION` in `tool/versions.env`, radar-tracked by `upgrade.sh`); mold comes from apt on CI. |
