...
| The test family on ghcr | The Rust tests job keeps its whole `target/debug` trees (every integration-test and bin executable, which rust-cache always dropped) plus the cargo registry on the GitHub Container Registry, keyed on the vendor revs with a `-latest` fallback so an engine-bump PR rebuilds only what changed; rust-cache is skipped for that family. Gigabytes per revision — fine on GHCR (public, uncapped, pruned at 14 days), impossible on the Actions cache. |
