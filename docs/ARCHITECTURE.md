...
| The cache | `Swatinem/rust-cache` archives `~/.cargo` + both vendored `target/` dirs per key. Key = rustc host + version, every `Cargo.lock`/`Cargo.toml` hash, the `CARGO`/`RUST` env, the make target (rows that build different triples or features never share) and both vendored submodule revs. `cache-workspace-crates: true` is required — the default drops the workspace's own crates, and the vendored engine IS the workspace crate. |
