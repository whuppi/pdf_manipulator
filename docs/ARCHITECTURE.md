...
- **Instance-based, lifecycle-controlled.** `final pdf = Pdf();` creates an instance with its own worker (Isolate on native, Web Worker on web). `pdf.kill()` disposes the worker and cancels all pending operations. No static shared state — consumers control lifecycle and parallelism by managing instances.
...
- **Vendor via submodule with patches.** pdf_oxide is a git submodule at `vendor/pdf_oxide`, pointing at a fork branch (`pdf_manipulator/0.3.47-patches`). The fork adds 23 C-ABI functions to the header that upstream hasn't shipped yet (open-from-bytes, password auth, watermark, image optimization, PDF/A validation, etc.). Each patch has a removal trigger — when upstream adds the equivalent, the patch is deleted. Full inventory in [`UPDATING.md`](UPDATING.md).
...
pdf_manipulator/
├── lib/
│   ├── pdf_manipulator.dart              ← barrel (one barrel, entire public API)
│   └── src/
│       ├── builder/                      ← PdfBuilder — create PDFs from scratch
│       │   └── pdf_builder.dart
│       ├── core/                         ← shared types (errors, models, value objects)
...
│       │   ├── pdf_signature.dart
│       │   └── search_result.dart
│       ├── document/                     ← Pdf instance + PdfDoc read-only handle
│       │   ├── pdf.dart
│       │   └── pdf_doc.dart
│       ├── editor/                       ← PdfEditor mutable handle
│       │   └── pdf_editor.dart
│       ├── ffi/                          ← FFI layer (generated + hand-written wrappers)
│       │   ├── native_bindings.g.dart    ← GENERATED by ffigen — never hand-edit
│       │   └── bindings.dart             ← PdfBindings — memory-safe wrappers
│       ├── page/
│       │   └── pdf_page_info.dart
│       └── platform/                     ← conditional import dispatch
...
├── vendor/
│   └── pdf_oxide/                        ← git submodule pinned to tag
├── hook/
│   └── build.dart                        ← compiles pdf_oxide → .dylib / .wasm
├── web_assets/                           ← WASM binary + JS glue for web
├── ffigen.yaml
├── docs/
...
Each `Pdf()` instance spawns its own background `Isolate`. The main isolate and the worker exchange `SendPort`s. `kill()` calls `Isolate.kill()` on the worker, cancelling all pending operations. Every operation:
...
Each `Pdf()` instance creates its own `web.Worker`. The WASM binary loads inside the worker. `kill()` calls `Worker.terminate()`, cancelling all pending operations. Every operation:
...
| Cleanup | `Isolate.kill()` via `pdf.kill()` | `Worker.terminate()` via `pdf.kill()` |
...
Stateful operations (`PdfEditor`, `PdfBuilder`) need handles that persist across multiple calls. Created via the `Pdf` instance:

- `PdfEditor(await pdf.openEditor(bytes))` — opens an editor handle on the instance's worker.
- `PdfBuilder(await pdf.createBuilder())` — creates a builder handle on the instance's worker.

The mechanism:

1. The worker holds a `Map<int, Pointer<Void>> handles` — the handle map.
2. `pdf.openEditor(bytes)` assigns a fresh integer `handleId`, sends a `WorkerMsg(op: Op.editorOpen, handleId: id, bytes: ...)`. The worker opens the editor and stores the pointer in `handles[id]`.
3. The main isolate receives the `handleId` and wraps it in a `_NativeEditorHandle`.
4. Each editor method (e.g. `setTitle`) sends a `WorkerMsg(op: Op.editorSetTitle, handleId: id, strArg: 'title')`. The worker's `_dispatch` looks up `handles[id]` and calls the FFI method.
5. `editor.dispose()` sends `Op.editorDispose` which removes the handle from the map and calls `b.editorFree(h)`.
6. On `pdf.kill()`, the worker frees all remaining handles before shutting down.
...
`Pdf`, `PdfEditor`, `PdfBuilder` are the public surface. `Pdf` is instance-based (`final pdf = Pdf()`); each instance owns its worker. All methods are instance methods that dispatch closures to the instance's platform worker. The closure receives `PdfBindings` and does the work. `PdfEditor` and `PdfBuilder` are created from a `Pdf` instance (`PdfEditor(await pdf.openEditor(bytes))`, `PdfBuilder(await pdf.createBuilder())`).
...
1. **Source compile** — if `vendor/pdf_oxide/Cargo.toml` exists (contributor with submodule checked out), compile from source via `cargo build` directly. No `native_toolchain_rust` — the hook calls cargo itself.
...
Pre-built WASM committed in `web_assets/`. Built by `tool/build_wasm.sh` or by the release CI. The WASM loads lazily on first PDF operation — app startup is unaffected.
