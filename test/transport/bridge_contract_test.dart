...
void main() {
  group('request encoding contract', () {
    ...
    test('field count follows op name', () {
      ...
      final fieldCount = ByteData.sublistView(bytes)
...
Uint8List _buildErrorResponse(String msg) {
  ...
  buf.add((ByteData(4)..setUint32(0, msgBytes.length, Endian.little))
      .buffer.asUint8List());
  buf.add(msgBytes);
...
Uint8List _buildOkResponse(Map<String, Object?> fields) {
  // Use the same encoder to build a response-shaped binary.
  // encodeRequest puts op+fields; for response we need status+fields.
  // Build manually to match the response format exactly.
  final buf = BytesBuilder();
  buf.addByte(1); // status ok
  final entries = fields.entries.where((e) => e.value != null).toList();
  buf.add((ByteData(2)..setUint16(0, entries.length, Endian.little))
      .buffer.asUint8List());
  ...
  switch (value) {
    ...
    case final List<Map<String, Object?>> v:
      ...
      for (final m in v) {
        ...
        buf.add((ByteData(4)..setUint32(0, nested.length, Endian.little)).buffer.asUint8List());
        buf.add(nested);
      }
  }
}
...
Uint8List _buildNestedMap(Map<String, Object?> m) {
  ...
  buf.add((ByteData(2)..setUint16(0, entries.length, Endian.little)).buffer.asUint8List());
  for (final e in entries) {
    _addField(buf, e.key, e.value!);
  }
  return buf.takeBytes();
