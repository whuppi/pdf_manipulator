...
//   - Rust-side internal buffering (reading 64KB chunks but accumulating
//     them into a Vec<u8> inside the engine). The transport looks O(1)
//     but Rust memory is O(n).
//
// THE TRUE O(1) VERIFICATION (not yet implemented):
//   Wire a tracking allocator (e.g. `tracking_allocator` or jemalloc
//   stats) into the Rust test harness. For each op:
//     1. Record allocator baseline before the op
//     2. Run the op on a LARGE file (50MB+)
//     3. Record peak allocation during the op
//     4. Assert peak < fixed bound (e.g. 5MB — way less than input)
//   If any op secretly buffers the full file, peak ≥ 50MB → test fails.
//   This catches Vec<u8> accumulation inside Rust that the Dart guard
//   can't see. Combine both: Dart guard catches transport violations,
//   Rust allocator check catches engine violations.
