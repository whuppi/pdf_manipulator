...
void main() {
  ...
  group('Pdf.open', () {
    test('opens minimal PDF — page count is 1', () async {
      final doc = await pdf.open(sourceOf(minimalPdf));
    ...
    test('reads Letter page dimensions', () async {
      final doc = await pdf.open(sourceOf(letterPdf));
    ...
    test('throws on garbage bytes', () async {
      expect(() => pdf.open(sourceOf(garbageBytes)), throwsA(isA<PdfError>()));
    });

    test('throws on empty bytes', () async {
      expect(() => pdf.open(sourceOf(emptyBytes)), throwsA(isA<PdfError>()));
    });

    test('throws on broken PDF', () async {
      expect(() => pdf.open(sourceOf(brokenPdf)), throwsA(isA<PdfError>()));
    });
  });
}
