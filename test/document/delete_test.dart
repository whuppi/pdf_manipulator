...
void main() {
  ...
  group('Pdf.deletePages', () {
    test('deleting 1 page from 3-page PDF leaves 2 pages', () async {
      ...
      await pdf.deletePages(sourceOf(threePagePdf), sink, pages: [1]);
    ...
    test('deleting 2 pages from 3-page PDF leaves 1 page', () async {
      final sink = TestPdfSink();
      await pdf.deletePages(sourceOf(threePagePdf), sink, pages: [0, 2]);
      final doc = await pdf.open(sourceOf(sink.takeBytes()));
    ...
    test('deleting first page shifts remaining pages', () async {
      ...
      final sink = TestPdfSink();
      await pdf.deletePages(sourceOf(threePagePdf), sink, pages: [0]);
      final doc = await pdf.open(sourceOf(sink.takeBytes()));
      expect(doc.pageCount, equals(2));
    ...
    test('deleting last page preserves earlier pages', () async {
      final sink = TestPdfSink();
      await pdf.deletePages(sourceOf(threePagePdf), sink, pages: [2]);
      final doc = await pdf.open(sourceOf(sink.takeBytes()));
      expect(doc.pageCount, equals(2));
  ...
  group('Pdf.movePage', () {
    test('move preserves page count', () async {
      final sink = TestPdfSink();
      await pdf.movePage(sourceOf(threePagePdf), sink, from: 0, to: 2);
      final doc = await pdf.open(sourceOf(sink.takeBytes()));
    ...
    test('move to same position preserves page count', () async {
      final sink = TestPdfSink();
      await pdf.movePage(sourceOf(threePagePdf), sink, from: 1, to: 1);
      final doc = await pdf.open(sourceOf(sink.takeBytes()));
      expect(doc.pageCount, equals(3));
    });
    ...
    test('result is re-openable', () async {
      final sink = TestPdfSink();
      await pdf.movePage(sourceOf(threePagePdf), sink, from: 2, to: 0);
      final doc = await pdf.open(sourceOf(sink.takeBytes()));
      expect(doc.pageCount, equals(3));
    });
  });
  ...
  group('PdfEditor.deletePage', () {
    test('reduces page count', () async {
      final editor = await Pdf.edit(sourceOf(threePagePdf));
