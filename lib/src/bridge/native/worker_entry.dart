...
// Creates NativeCallable.listener callbacks for the Rust pool threads.
//
...
import 'package:pdf_manipulator/src/bridge/ffi/bridge_bindings.dart' as bridge;
import 'package:pdf_manipulator/src/bridge/native/shared_buffer.dart';
import 'package:pdf_manipulator/src/bridge/native/worker_isolate.dart' as helpers;
...
void workerEntryPoint(SendPort mainPort) {
  ...
  // Initialize the bridge (stores Dart_PostCObject + creates thread pool)
  helpers.initBridge();
  ...
  workerPort.listen((message) async {
    ...
    try {
      if (op == 'stream') {
        // Streaming ops send multiple items directly to responsePort
        await _handleStream(args,
          opCode: args['opCode'] as int,
...
Future<Object?> _dispatch(String op, Map<String, Object?> args) async {
  switch (op) {
    ...
    case 'read':
      return _handleRead(args, opCode: args['opCode'] as int);
    case 'embedFile':
    ...
    case 'editorPageCount':
      return Uint8List.fromList([bridge.bridgeEditorPageCount(args['handleId'] as int)]);
...
Future<Uint8List> _handleOpen(Map<String, Object?> args) async {
  ...
  // Create NativeCallable.listener for read notifications
  final listener = helpers.createReadListener(readBuf, sourcePort);

  // Create ReceivePort for the result (allo-isolate posts here)
  final resultPort = ReceivePort();
  final nativePort = resultPort.sendPort.nativePort;
