...
/// Create a document by parsing Markdown and writing to any `Write` destination.
    // office_kit patch: Write+Seek → Write (streaming ZIP)
pub fn create_from_markdown_to_writer<W: Write>(
    markdown: &str,
    format: DocumentFormat,
    writer: W,
) -> Result<()> {
...
/// Create a document from IR and write to any `Write` destination.
pub fn create_from_ir_to_writer<W: Write>(
    ir: &DocumentIR,
    format: DocumentFormat,
    writer: W,
) -> Result<()> {
