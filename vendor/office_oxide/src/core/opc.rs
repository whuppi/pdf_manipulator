...
// ── office_kit patch: streaming ZIP (Write-only, no Seek) ──
...
// ── office_kit patch: OpcWriter uses streaming ZIP (Write-only) ──
/// Writer for creating OPC packages (ZIP-based).
/// Uses streaming ZIP (data descriptors, no backward seeking) so the
/// output can be a `Write`-only sink — O(1) memory, any destination.
pub struct OpcWriter<W: Write> {
    writer: ZipWriter<StreamWriter<W>>,
...
impl<W: Write> OpcWriter<W> {
    /// Create a new OPC package writer wrapping any `Write` sink.
    pub fn new(writer: W) -> Result<Self> {
        Ok(Self {
            writer: ZipWriter::new_stream(writer),
    ...
    pub fn finish(mut self) -> Result<W> {
        ...
        let stream_writer = self.writer.finish()?;
